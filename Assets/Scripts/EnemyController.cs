...
namespace Paraverse.Enemy
{
    ...
    public class EnemyController : MonoBehaviour
    {
        ...
        [SerializeField, Tooltip("The patrolling speed of the character.")]
        private float patrolSpeed = 1.5f;
        [SerializeField, Tooltip("The sprint speed of the character.")]
        private float pursueSpeed = 3f;
        [SerializeField, Tooltip("The rotation speed of the character.")]
        ...
        /// <summary>
        /// Responsible for rotating the character towards the target.
        /// </summary>
        /// <param name="target"></param>
        private void RotationHandler(Vector3 target)
        {
            Vector3 targetDir = (target - transform.position).normalized;
            Vector3 lookDir = Vector3.RotateTowards(transform.forward, targetDir, rotSpeed * Time.deltaTime, 0.0f);
            lookDir = new Vector3(lookDir.x, 0.0f, lookDir.z);
            transform.rotation = Quaternion.LookRotation(lookDir);
        }

        /// <summary>
        /// Responsible for moving character controller to target position.
        /// </summary>
        /// <param name="target"></param>
        private void SetDesination(Vector3 target)
        {
            Vector3 movePos = (target - transform.position).normalized;
            movePos += Physics.gravity;
            controller.Move(movePos * curSpeed * Time.deltaTime);
        }
        #endregion
        ...
        private void Patrol()
        {
            ...
            else
            {
                ...
                SetDesination(waypoints[curWaypointIdx].position);
            }
            RotationHandler(waypoints[curWaypointIdx].position);
        }
        ...
        private void PursueTarget()
        {
            if (pursueTarget != null)
            {
                ...
                {
                    curSpeed = pursueSpeed;
                    SetDesination(pursueTarget.position);
                    Debug.Log("Pursuing");
                }
                RotationHandler(pursueTarget.position);
            }
        }
