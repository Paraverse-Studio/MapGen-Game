...
public class FirebaseDatabaseManager : MonoBehaviour
{
    ...
    public delegate void UpdateAchievementCallback(AchievementsModel achievements);
    public delegate void PostAchievementFailureCallback();
    public delegate void GetAchievementCallback(AchievementsModel model);
    public delegate void GetAchievementsCallback(Dictionary<string, AchievementsModel> model);
    ...
    public void PostAchievements(AchievementsModel model, UpdateAchievementCallback callback)
    {
        string username = model.Username;

        RestClient.Put<AchievementsModel>($"{databasePath}{achievementsPath}/{username}.json", model)
          .Then(response =>
          {
              callback?.Invoke(response);
          })
    ...
    public void GetAchievement(string username, GetAchievementCallback onSuccessCallback, PostAchievementFailureCallback onFailureCallback)
    {
        try
        {
            RestClient.Get<AchievementsModel>($"{databasePath}{achievementsPath}/{username}.json")
                .Then(response =>
                {
                    onSuccessCallback?.Invoke(response);
                })
    ...
    public void GetAchievements(GetAchievementsCallback callback)
    {
        try
        {
            RestClient.Get($"{databasePath}{achievementsPath}.json")
              .Then(response =>
              {
                  var responseJson = response.Text;
                  ...
                  var data = fsJsonParser.Parse(responseJson);
                  object deserialized = null;
                  serializer.TryDeserialize(data, typeof(Dictionary<string, AchievementsModel>), ref deserialized);

                  var achievementsDictionary = deserialized as Dictionary<string, AchievementsModel>;

                  callback?.Invoke(achievementsDictionary);
              })
              .Catch(error =>
              {
                  Debug.Log("Error: " + error);
